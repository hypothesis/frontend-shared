// This file was auto-generated using scripts/generate-icons.js
import type { JSX } from 'preact';

export type TagAltFilledIconProps = JSX.SVGAttributes<SVGSVGElement>;

/**
 * Icon generated from tag-alt-filled.svg
 * @deprecated - Use `TagFilled` instead
 */
export default function TagAltFilledIcon(props: TagAltFilledIconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="16"
      height="16"
      aria-hidden="true"
      viewBox="0 0 16 16"
      data-component="TagAltFilledIcon"
      {...props}
    >
      <g fill-rule="evenodd">
        <path fill="none" d="M0 0h16v16H0z" />
        <path
          fill="currentColor"
          d="M5 3a5 5 0 1 1 0 10A5 5 0 0 1 5 3m6 0a5 5 0 0 1 0 10 1 1 0 0 1-.117-1.993L11 11a3 3 0 0 0 0-6 1 1 0 0 1 0-2"
        />
      </g>
    </svg>
  );
}
