...
describe('Select', () => {
  ...
  context('when listbox does not fit in available space', () => {
    ...
    it('never renders a listbox bigger than the viewport', async () => {
      ...
      assert.isTrue(listboxWidth < window.window.innerWidth);
    });

    [
      { name: 'long name'.repeat(50), shouldGrow: true },
      { name: 'short name', shouldGrow: false },
    ].forEach(({ name, shouldGrow }) => {
      it('makes listbox grow to the left if there is space', async () => {
        const wrapper = createComponent(
          { buttonContent: 'Select a value' },
          {
            items: ['1', '2', '3'].map(id => ({ id, name })),
        ...
        const listbox = await getOpenListbox(wrapper);
        const listboxStyleLeft = listbox.getDOMNode().style.left;
        const listboxLeft = Number(listboxStyleLeft.replace('px', ''));
        const { left: buttonLeft } = getToggleButton(wrapper)
          .getDOMNode()
          .getBoundingClientRect();

        assert.equal(listboxLeft < buttonLeft, shouldGrow);
      });
    });
    [
      { name: 'long name'.repeat(50), shouldGrow: true },
      ...
      it('makes right Select listbox grow to the right if there is space', async () => {
        const wrapper = createComponent(
          { right: true, buttonContent: 'Select a value' },
          {
            items: ['1', '2', '3'].map(id => ({ id, name })),
          },
        );
        ...
        // 8 is the minimum `left` value that acts as a gap with the edge of the
        // viewport
        assert.equal(listboxLeft === 8, shouldGrow);
      });
    });
  });
