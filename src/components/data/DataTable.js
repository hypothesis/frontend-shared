 ...
 *
 * @typedef {( row:Row, field:string) => Children|null} RenderItem
 * @typedef {( row: Row ) => void } RowCallback
 *
 ...
 * @prop {RowCallback} [onConfirmRow] - Callback when a row is "confirmed" by
 *   double-click or pressing "Enter"
 * @prop {RowCallback} [onSelectRow] - Callback when a row is "selected" by
 *   focus or click
 * @prop {RenderItem} [renderItem] - Callback to render an individual table cell
 ...
 *
 * @param {CommonProps & DataTableProps & HTMLAttributes} props
 */
const DataTableNext = function DataTable({
   ...
   * @param {string} field
   */
  function formatDataRowItem(row, field) {
    return renderItem?.(row, field) ?? row[field];
  }
